...
def ai_response(prompt, networking = False, temperature =.5):
    ...
    if networking:
        ...
        if first_word in update_strings + general_strings + reminder_strings:
            ...
            if not verified:
                ...
                print("*** UPDATING USER ***")
                prompt = """Pretend you building a JSON file called "People" that is in the following rough format (comments in "[]"):\n\n {"People": {"[PERSONS NAME]": {"School": "[SCHOOL]","Location": "[LOCATION]","Interests":"[INTEREST]", "Fun Facts":"[FUN FACTS]", "[OTHER RELEVANT FIELD NAME]":"[OTHER DATA]" }}}  \n\nBased on this design, what fields and keys would you make out of the following dialog:""" + prompt+"\n\Respond with only JSON, and with facts you are certain about. Do not respond with null fields, and add new fields as necessary."
    ...
    if networking and first_word in update_strings:
        ...
        print("******** JSON UPDATED ********")    
    if prompt.lower() == 'this is wrong':
        with open("prompt_errors.txt", "a") as f:
